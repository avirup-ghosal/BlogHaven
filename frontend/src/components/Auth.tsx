import { SignupInput } from "@avirup2023/medium-common";
...
async function sendRequest(){
    try{
        ...
        const response=await axios.post(`${BACKEND_URL}/api/v1/user/${type==="signup"?"signup":"signin"}`, JSON.stringify(postInputs));
    ...
    return <div className="h-screen flex justify-center flex-col">
        ...
        {type==="signup"?<LabelledInput label="Name" placeholder="Avirup Ghosal..." onChange={(e)=>{
            setPostInputs(
                {
                ...postInputs,
                name:e.target.value
        ...
        <LabelledInput label="Username" placeholder="avirupghosal@gmail.com" onChange={(e)=>{
            setPostInputs(
                {
                ...postInputs,
        ...
        <LabelledInput label="Password" type="password" placeholder="Avirup Ghosal..." onChange={(e)=>{
            setPostInputs(
                {
                ...postInputs,
...
function LabelledInput({label,type,placeholder,onChange}:LabelledInputType){
    return <div>
        <label className="block mb-2 text-sm font-medium text-black text-left">{label}</label>
